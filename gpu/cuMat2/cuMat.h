...
public:
    ...
    void new_matrix(int rows, int cols) {
	    ...
	    mHost = (float*)calloc(nBytes, sizeof(float));     
    ...
    void memHostToDevice() {
        ...
        if (error != cudaSuccess) printf("246: memHostToDevice cudaMemcpy error\n");
    }
	...
	memcpy(mHost, v, rows * cols * sizeof(*mDevice));
	
        cudaError_t error = cudaMemcpy(mDevice, v,
                rows * cols * sizeof(*mDevice), cudaMemcpyHostToDevice);
        if (error != cudaSuccess)
            printf("272: memSetHost cudaMemcpy error\n");
    }
	...
	float *tmp;
	tmp = (float*)calloc(a.cols * a.rows, sizeof(float));
	cublasGetVector(a.rows * a.cols, sizeof(float), a.mDevice, 1, tmp, 1 );
	mHost = (float*)calloc(cols * rows, sizeof(float));
	
	memcpy(mHost, tmp, cols * rows * sizeof(float));
	...
	/*
	r.mHost = (float*)calloc(r.cols * r.rows, sizeof(float));

	for(int i = 0; i < r.cols * r.rows; i++)
	  r.mHost[i] = mHost[i] + b.mHost[i];
	
	cudaError_t error = cudaMemcpy(r.mDevice, r.mHost, rows * cols * sizeof(*mDevice), cudaMemcpyHostToDevice);
	if (error != cudaSuccess) printf("538: memHostToDevice cudaMemcpy error\n");      

	float *tmp;
	tmp = (float*)calloc(r.cols * r.rows, sizeof(float));
        cublasGetVector(r.rows * r.cols, sizeof(float), r.mDevice, 1, tmp, 1 );

	memcpy(r.mHost, tmp, r.rows * r.cols * sizeof(*r.mDevice));
	free(tmp);
	*/
    }
	...
	for(int i = 0; i < r.cols * r.rows; i++)
	  r.mHost[i] = mHost[i] * r.mHost[i];
	
	cudaError_t error = cudaMemcpy(r.mDevice, r.mHost, rows * cols * sizeof(*mDevice), cudaMemcpyHostToDevice);
	if (error != cudaSuccess) printf("[mul_plus] memHostToDevice cudaMemcpy error\n");      
	*/
	...
	*/
	...
	cudaError_t error = cudaMemcpy(r.mDevice, r.mHost, rows * cols * sizeof(*mDevice), cudaMemcpyHostToDevice);
	if (error != cudaSuccess) printf("658: memHostToDevice cudaMemcpy error\n");      

	r.mHost = (float*)calloc(r.cols * r.rows, sizeof(float));
	memcpy(r.mHost, tmp, r.cols * r.rows * sizeof(float));
	*/
    }
    ...
    void dot_plus(const cuMat &b, cuMat &r) {
		...
		float *tmp; 
		tmp = (float*)calloc(b.cols * b.rows, sizeof(float));
	        cublasGetVector(b.rows * b.cols, sizeof(float), b.mDevice, 1, tmp, 1 );
		free(tmp);
		...
		cudaError_t error = cudaMemcpy(r.mDevice, r.mHost, r.rows * r.cols * sizeof(*mDevice), cudaMemcpyHostToDevice);
		if (error != cudaSuccess) printf("700: memHostToDevice cudaMemcpy error\n");      
	...
	*/
	...
	memcpy(mHost, tmp, r.rows * r.cols * sizeof(float));
	*/
    }
	...
	/*
	for(int i = 0; i < r.cols * r.rows; i++)
	  r.mHost[i] = 1.0f/(1.0f + std::exp(-mHost[i])); 
